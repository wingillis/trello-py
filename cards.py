...
class Cards(object):
    ...
    def update(self, card_id, name=None, desc=None, closed=None, idList=None, due=None, pos=None):
        resp = requests.put("https://trello.com/1/cards/%s" % (card_id), params=dict(key=self._apikey, token=self._token), data=dict(name=name, desc=desc, closed=closed, idList=idList, due=due, pos=pos))
        resp.raise_for_status()
        return json.loads(resp.content.decode('utf-8'))
    ...
    def new_attachment(self, card_id, url=None, name=None,f=None):
        if f:
            files = {'file': f}
            resp = requests.post("https://trello.com/1/cards/%s/attachments" % (card_id), params=dict(key=self._apikey, token=self._token), files=files)
        else:
    ...
    def delete_attachment(self, card_id, attach_id):
        resp = requests.delete("https://trello.com/1/cards/%s/attachments/%s" % (card_id, attach_id), params=dict(key=self._apikey, token=self._token), data=None)
        resp.raise_for_status()
        return json.loads(resp.content.decode('utf-8'))
